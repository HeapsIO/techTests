class Cursor implements hxbit.NetworkSerializable {
	...
	@:s #if hxbit_visibility @:visible(server) #end public var test : Array<Int> = [1,2,3];
...
class Context implements hxbit.NetworkSerializable {
	@:s public var cursors : Array<Cursor> = [];
	public function new() {
	}
}
	...
	override function init() {
		...
		try {
			...
			host.onMessage = function(c,uid:Int) {
				...
				context.cursors.push(cursorClient);
				c.ownerObject = cursorClient;
				c.sync();
			};
			log("Server Started");
			context = new Context();
			host.rootObject = context;
			start();
		} catch( e : Dynamic ) {
